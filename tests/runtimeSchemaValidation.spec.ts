...
describe('runtimeSchemaValidation', () => {
  describe('default types', () => {
    ...
    test('0.0 - missing string fails', async () => {
      await validateSimpleDataAgainstSchema(z.object({ s: z.string() }), { s: undefined }, { success: false })
    })
    ...
    test('0.2 - missing number fails', async () => {
      await validateSimpleDataAgainstSchema(z.object({ c: z.number() }), { c: undefined }, { success: false })
    })
