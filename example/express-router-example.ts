...
const zDateISORequired = zDual(
  ...
  z.date().transform((d) => d.toISOString()).pipe(z.string()),
);

const zDateISO = zDual(
  z.string().datetime().transform((s: string) => new Date(s)).pipe(z.date()).optional(),
  z.date().transform((d) => d.toISOString()).pipe(z.string()).optional(),
);
...
users.get(
  ...
  apiDoc({ query: ListQuery, returns: ListUsersOut })((req, res) => {
      ...
      const filtered = q
        ? all.filter((u) => [u.email, u.name].some((s) => s.toLowerCase().includes(q)))
        : all;
      const page = filtered.slice(offset, offset + limit);

      res.transformSend({ data: page.map((u) => ({ ...u, birthday: u.birthday ?? undefined })), total: filtered.length });
    },
  ),
);
